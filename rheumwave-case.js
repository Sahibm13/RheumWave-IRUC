...
if (CASE_ID === undefined) {
  console.error('RheumWave: this page did not set window.RW_CASE_ID before ' +
                'loading rheumwave-case.js. Scores cannot be recorded.');
}
...
if (typeof RWSync === 'undefined') {
  console.error('RheumWave: rheumwave-sync.js did not load, so scores cannot be ' +
                'saved. It must be loaded before rheumwave-case.js.');
}
...
function openLightbox(imgEl) {
  ...
  document.getElementById('usLightboxImg').src = imgEl.src;
  document.getElementById('usLightboxImg').alt = imgEl.alt || '';
  document.getElementById('usLightboxCaption').textContent = imgEl.alt || '';
  box.classList.add('open');
  document.body.style.overflow = 'hidden';
}

function closeLightbox(e) {
  // clicking the enlarged image itself should NOT close; overlay/close button does
  if (e && e.target && e.target.id === 'usLightboxImg') return;
